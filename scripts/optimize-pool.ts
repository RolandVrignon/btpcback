...
async function optimizeConnectionPool() {
  console.log(
    '🔍 Analyse de votre base de données pour optimiser le pool de connexions...',
  );
  ...
  try {
    ...
    console.log(
      "📊 Récupération des informations sur l'instance PostgreSQL...",
    );
    ...
    const sharedBuffersResult = await prisma.$queryRaw<
    ...
    if (currentPoolSize) {
      ...
      if (currentPoolSize < minPoolSize) {
        console.log(
          '⚠️ Votre pool actuel est trop petit. Augmentez-le pour améliorer les performances.',
        );
      } else if (currentPoolSize > recommendedPoolSize * 1.2) {
        console.log(
          '⚠️ Votre pool actuel est probablement trop grand. Réduisez-le pour économiser des ressources.',
        );
      } else {
        ...
        if (match && match[1]) {
          ...
          if (connectionLimitRegex.test(currentUrl)) {
            ...
            const newUrl = currentUrl.replace(
              connectionLimitRegex,
              `?connection_limit=${recommendedPoolSize}`,
            );
            console.log(
              `1. Modifiez votre DATABASE_URL dans .env pour inclure connection_limit=${recommendedPoolSize}`,
            );
          } else {
            // Ajouter la limite
            ...
            console.log(
              `1. Ajoutez "?connection_limit=${recommendedPoolSize}" à votre DATABASE_URL dans .env`,
            );
          }
        }
      } else {
    ...
    console.log(
      `2. Mettez à jour votre PrismaService avec cette configuration :`,
    );
    console.log(`
   constructor() {
     super({
    ...
    // 3. Surveillance
    console.log(
      `3. Surveillez régulièrement l'utilisation des connexions avec le script check-connections.sh`,
    );
  } catch (error) {
...
optimizeConnectionPool().catch((error) => {
